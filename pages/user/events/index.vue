									...
									<td class="tdScore text-left">
								  	...
								  	<td class="tdScore2 text-center">
										<nuxt-link :to="'/user/events/' + event.id" class="linkEvent">
											...
											<span>-</br><span :class="[event.status === 'IN PLAY' || event.status === 'ADDED TIME' ? 'in_play' : '']">{{ event.status }}</span></span>
											<transition name="fade" mode="out-in" :duration="{ enter: 3000, leave: 2000 }">
								...
								<tr class="borderResultat" v-for="event of events">
									<td class="tdScore text-left">
								  	...
								  	<td class="tdScore2 text-center">
										<nuxt-link :to="'/user/events/' + event.id" class="linkEvent">
											<transition name="fade" mode="out-in" :duration="{ enter: 3000, leave: 2000 }">
												<span class="" style="float: left; padding: 15px;" :key="getHomeTeamScore(event.score)" v-if="event.status === 'IN PLAY' || event.status === 'HALF TIME BREAK' || event.status === 'ADDED TIME' || event.status === 'FINISHED'">
													{{ getHomeTeamScore(event.score) }}
												</span>
											</transition>
											<span>-</br><span :class="[event.status === 'IN PLAY' || event.status === 'ADDED TIME' ? 'in_play' : '']">{{ event.status }}</span></span>
											<transition name="fade" mode="out-in" :duration="{ enter: 3000, leave: 2000 }">
												<span class="" style="float: right; padding: 15px;" :key="getVisitorTeamScore(event.score)" v-if="event.status === 'IN PLAY' || event.status === 'HALF TIME BREAK' || event.status === 'ADDED TIME' || event.status === 'FINISHED'">
													{{ getVisitorTeamScore(event.score) }}
												</span>
											</transition>
										</nuxt-link>
